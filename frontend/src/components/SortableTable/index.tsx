...
export interface GenericAction<T> extends Action {
    ...
    callback: (args: T[keyof T]) => void;  // callback function
}
...
const SortableTable = <T,>({ data, header, sortFunc, actions }: GenericProps<T>) => {
    ...
    const changeSortKey = (e: SyntheticEvent, sortKey: keyof T | undefined) => {
        if (e.type === 'click') {
            setSortBy(v => ({ key: sortKey, order: !v.order }));
        }
    ...
    const isTableCell = (el: unknown): boolean => {
        if (!el) return false;
        const element = el as ReactElement;
        return element?.type !== undefined && (
            (element.type as React.ComponentType).name === 'TableCell' ||
            (element.type as React.ComponentType).displayName === 'TableCell' ||
            element.props?.as === 'td'
        );
    };
